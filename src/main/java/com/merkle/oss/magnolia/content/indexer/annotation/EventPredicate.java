    ...
    class SystemExcludingPredicates implements EventPredicate {
        ...
        public boolean test(final Event event) {
            try {
                ...
                return !path.startsWith("/"+ JcrConstants.JCR_SYSTEM) && FilteredEventListener.ACTIVATION_EXCLUDING_PREDICATE.evaluate(event) && !BLACKLIST_PATTERN.matcher(path).find();
            } catch (Exception e) {
